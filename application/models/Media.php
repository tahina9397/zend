...
class Application_Model_Media { 
	...
	public static function unlink($id){
		...
		$filename = $media['filename'];
		$media_path = $media_path_orig = UPLOAD_PATH . "images/medias/" . $filename;
	...
	public static function getMediaWidth($id, $size_name = "source"){
		...
		if($media){
			...
			$media_path = UPLOAD_PATH . "images/medias/" . $filename;
			if(file_exists($media_path) && is_file($media_path)){
 	...
 	public static function getImageByResourceTypeAndResourceId($resource_type,$resource_id, $size_name = "source")
 	{
 		...
 		if(!empty($medias)){
 			foreach ($medias as $k => $item) {
 				$media_path = UPLOAD_PATH . "/images/medias/" . $item['filename'];	
 				...
 				if(file_exists($media_path) && is_file($media_path)){
 					...
 					$media[$k]["id"] = $item["id"] ;
 					$media[$k]["url"] = UPLOAD_URL . "/images/medias/" . $filename;
 				}	
 			}
 		}
