$(document).ready(function() {
 ...
 //    });
	...
	$('.change-lang').click(function(){
		$.post(baseUrl+"/translations/changelang"
			,{ 
				"action" : 'changelang'
			}, 
			function(data){
	...
	$(".deletecover").live("click", function(){
    	...
    	$.post(baseUrl + "/medias/deletecover", {
			"id_item"      : id_item
			,"module_item" : module
			,"type"        : type
			,"media_id"    : id_media
	...
	/*fancybox*/
	$(document).on("click","a.featured-image",function() {
    	var id = $(this).attr("data-id");
    	var _module = $(this).attr("data-module");
    	var type = $(this).attr("data-type");
    	var id_media = $(this).attr("data-id-media");

    	$.post(baseUrl + "/medias/checkmedias", {
		}, function(data) {
			if (data) {
				$.fancybox.open({
				    src  : baseUrl+"/medias/browse/?item_id="+id+"&item_module="+_module+"&item_type="+type+"&id_media="+id_media
				    ,type : 'iframe'
				    ,opts:{
				        iframe:{
				            css:{
				...
				Swal.fire('Veuillez ajouter un média dans le menu Média') ;
			}
		}, 'json');
...
// 	        ,width    : '96%'
// 	        ,height   : '94%'
// 			,href     : baseUrl+'/medias/medias/format/html'
// 	        ,type     : 'ajax'
// 	        ,ajax: { type: "POST", data : "item_id="+id+"&item_module="+_module+"&item_type="+type+"&id_media="+id_media}         
//       	});
// }
...
function updateState()
{
	$('#statut').live('change',function(){
        ...
        $.post('/ajax/updatestate?resource_id='+resource_id+'&resource_type='+resource_type+'&activate='+activate,'json');
    });
